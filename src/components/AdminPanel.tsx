...
import { getIsFirebaseConfigured } from'../firebase';
import {
 Settings,
 Package,
 ShoppingBag,
...
'Payment Method','Payment Status','Order Status',
 ...
 setProdImageUploadError('Unsupported file type. Please upload JPG, PNG, WebP, GIF, or SVG.');
 return;
 }
 ...
 setProdImageUploadError(`Image too large. Maximum size is ${maxSizeMB}MB. Tip: resize to 600×600px first.`);
 return;
 }
 ...
 reader.onerror = () => {
 setProdImageUploadError('Failed to read file. Please try again.');
 };
 ...
 if (!allowedTypes.includes(file.type)) {
 setBrandLogoUploadError('Unsupported format. Use PNG, SVG, WebP, or JPG.');
 return;
 }
 if (file.size > 2 * 1024 * 1024) {
 setBrandLogoUploadError('File too large. Maximum logo size is 2MB.');
 return;
 }
 ...
 reader.onerror = () => {
 setBrandLogoUploadError('Failed to read file. Please try again.');
 };
 reader.readAsDataURL(file);
 };
