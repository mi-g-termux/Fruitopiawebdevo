...
function Step6Install({
  ...
  const runInstallFromStep3 = useCallback(async () => {
    ...
    // Sub-step 3 — Seed store data (single batched write — one round trip)
    markRunning(3, 'Setting up store data...')
    try {
      const db    = getDb()
    ...
    // Sub-step 4 — Create admin account
    markRunning(4, 'Creating admin account...')
    try {
      const db = getDb()
      await withTimeout(
    ...
    // Sub-step 5 — Save store settings (batched)
    markRunning(5, 'Saving store settings...')
    try {
      const db     = getDb()
    ...
    // Sub-step 6 — Finalise
    markRunning(6, 'Finalising installation...')
    try {
      const db = getDb()
      await withTimeout(
  ...
  const runInstall = useCallback(async () => {
    ...
    if (saved) {
      ...
      setInstallProgress(p => ({ ...p, status: 'awaiting-upload' }))
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
